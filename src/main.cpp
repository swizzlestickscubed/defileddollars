...
#if defined(NDEBUG)
# error "DefiledDollars cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "DefiledDollars Signed Message:\n";
...
{
    // DefiledDollars: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DefiledDollars: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DefiledDollars: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DefiledDollarsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DefiledDollarsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DefiledDollarsMiner(CWallet *pwallet)
{
    printf("DefiledDollarsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("defileddollars-miner");
    ...
    try { loop {
        ...
        printf("Running DefiledDollarsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DefiledDollarsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DefiledDollarsMiner, pwallet));
}
